...
- **FortiClient can silently ignore `Connect()` calls after a disconnect it classifies as "manual."** Confirmed in a live end-to-end test: after a drop, FortiClient's own trace log logged *"was manually disconnected, so not requesting a connection"*, and every subsequent `Connect()` call — five from the watchdog's retry ladder, plus one issued completely standalone, outside the watchdog entirely, to rule out a bug on this app's side — failed to bring the tunnel up, with FortiClient's log showing zero response to any of them. A genuine mouse click on the FortiClient tray icon reconnected it immediately. The watchdog's grace period, single-flight, and backoff/give-up logic all behaved correctly throughout (retried on schedule, gave up cleanly after 5 attempts, never got stuck) — this is a FortiClient-side limitation, not a bug here, but it means some disconnects genuinely cannot be recovered by this tool (or by any COM automation) and need a real click from the user. If you see repeated `GaveUp` in the activity log, try the tray icon directly before assuming something is broken.
- **Download throughput can show `n/a` on some FortiClient tunnel adapters.** Confirmed directly against Windows' own interface counters: on the tested machine, the FortiClient virtual adapter's `BytesSent` climbs normally but `BytesReceived` sits at a permanent 0 for the adapter's entire lifetime — not just a quiet moment, the cumulative OS counter itself never moves — while the physical Wi-Fi adapter right next to it reports both directions correctly. This is a driver-level limitation of that specific virtual adapter, not something readable around from user-mode. After a few upload-only samples with no download ever observed, the GUI and CLI both switch from a misleading "0 B/s" to "n/a (not reported by adapter)"; upload throughput and cumulative totals are unaffected and remain accurate.
